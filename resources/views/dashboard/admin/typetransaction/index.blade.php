    ...
    <div class="content">
        ...
        <table class="table is-striped is-hoverable is-fullwidth is-narrow">
            ...
            @foreach($datas as $data)
                <tr>
                    <td>{{ $no++ }}</td>
                    <td><a href="{{ route('typetransaction.show',$data->id) }}">{{ $data->name }}</a></td>
                    <td>
                        <form action="{{ route('typetransaction.destroy', $data->id) }}" method="post">
                            {{ csrf_field() }}
                            {{ method_field('DELETE') }}
                            <div class="field is-grouped">
                                <p class="control">
                                    <a class="button" type="submit" href="{{ route('typetransaction.edit',$data->id) }}">Edit</a>
                                </p>
                                <p class="control">
